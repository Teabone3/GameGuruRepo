	...
	if ( g_PlayerHealth>0 or GetGamePlayerControlStartStrength() == 0 ) then 
		...
		if ( GetFireModeSettingsJamChance() ~= 0 ) then 
			...
			if ( GetGamePlayerStateFiringMode() == 1 and GetFireModeSettingsJammed() == 0 ) then 
				...
				if ( GetFireModeSettingsShotsFired()>tchance and GetWeaponAmmo(GetGamePlayerStateWeaponAmmoIndex()+GetGamePlayerStateAmmoOffset())>1 ) then 
...
function gameplayercontrol.control()
	...
	if ( GetGamePlayerControlFootfallType() ~= -1 ) then 
		if ( GetGamePlayerControlMovement() == 0 or GetGamePlayerControlJumpMode() == 1 ) then 
			...
			-- only use 'protagonist' footfalls if flagged
			tusingprotagfootfalls = 0
			ttte=GetGamePlayerControlThirdpersonCharactere()
			if ( GetGamePlayerControlThirdpersonEnabled() == 1 and GetEntityFootfallMax(ttte) > 0 ) then tusingprotagfootfalls = 1 end
			if ( tusingprotagfootfalls == 1 ) then 
				-- third person uses protagonist animation to trigger footfalling
				tttobjframe=0.0
				tttobj=GetEntityElementObj(ttte)
				if ( tttobj>0 ) then 
				...
				for twalktypes = 0, GetEntityFootfallMax(ttte)-1, 1 do
					ttleftframe = GetEntityFootfallKeyframe(ttte,twalktypes,0) 
					ttrightframe = GetEntityFootfallKeyframe(ttte,twalktypes,1) 
				...
				if GetGamePlayerControlThirdpersonEnabled() == 1 then
					-- a decent approximation which can be controlled from the player properties (or use FPE with footfall info)
					ttFootfallPaceMultiplier = 1.0/(GetGamePlayerControlFootfallPace()/5.0)
				else
				...
				if ( GetGamePlayerControlPlrHitFloorMaterial() ~= 0 ) then 
					...
					-- Make sure we get one downhill, when we hit the ground again.
					if g_FootFallTimer < 315 and GetGamePlayerControlFootfallCount() == 1 then SetGamePlayerControlFootfallCount(0) end
					ttAddWobbleStep = GetGamePlayerControlWobbleSpeed()*GetElapsedTime()*GetGamePlayerControlBasespeed()*GetGamePlayerControlSpeedRatio()*ttFootfallPaceMultiplier*ttWeaponMoveSpeedMod
					g_FootFallTimer = g_FootFallTimer + ttAddWobbleStep
				end
			end
